...
OAUTH2_CLIENT_SECRET=$(az keyvault secret show --vault-name fis-aat -o tsv --query value --name idam-secret)
...
=======
  code=$(curl --insecure --fail --show-error --silent -X POST --user "$email:$password" "${IDAM_API_BASE_URL}/oauth2/authorize?redirect_uri=${REDIRECT_URI}&response_type=code&client_id=ds-ui" -d "" | docker run --rm --interactive stedolan/jq -r .code)
  idamToken=$(curl --insecure --fail --show-error --silent -X POST -H "Content-Type: application/x-www-form-urlencoded" --user "fis:${OAUTH2_CLIENT_SECRET}" "${IDAM_API_BASE_URL}/oauth2/token?code=${code}&redirect_uri=${REDIRECT_URI}&grant_type=authorization_code" -d "" | docker run --rm --interactive stedolan/jq -r .access_token)
>>>>>>> origin/master
...
=======
  caseId=$(curl --insecure --fail --show-error --silent -X GET ${CCD_BASE_URL}/citizens/$userId/jurisdictions/FIS/case-types/A58/cases -H "Authorization: Bearer $idamToken" -H "Content-Type: application/json" -H "ServiceAuthorization: Bearer $serviceToken" | docker run --rm --interactive stedolan/jq '.[0].id')
>>>>>>> origin/master
...
<<<<<<< HEAD
  curl --insecure --fail --show-error --silent -X POST http://ccd-data-store-api-aat.service.core-compute-aat.internal/citizens/$userId/jurisdictions/ADOPTION/case-types/A58/cases/$caseId/events -H "Authorization: Bearer $idamToken" -H "Content-Type: application/json" -H "ServiceAuthorization: Bearer $serviceToken" -d '{"event_token" : "'${eventToken}'","data":{"applicant1FirstName": "'${firstName}'","applicant1LastName":"'${lastName}'","applicant1Email" : "'${email}'"},"event" :{"id" :"patchCase"}}'
=======
  curl --insecure --fail --show-error --silent -X POST http://ccd-data-store-api-aat.service.core-compute-aat.internal/citizens/$userId/jurisdictions/FIS/case-types/A58/cases/$caseId/events -H "Authorization: Bearer $idamToken" -H "Content-Type: application/json" -H "ServiceAuthorization: Bearer $serviceToken" -d '{"event_token" : "'${eventToken}'","data":{"applicant1FirstName": "'${firstName}'","applicant1LastName":"'${lastName}'","applicant1Email" : "'${email}'"},"event" :{"id" :"patchCase"}}'
>>>>>>> origin/master
done
